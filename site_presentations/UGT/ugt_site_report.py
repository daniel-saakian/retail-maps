...
from ugt_proximity import nearest_ugt_locations
from ugt_model import load_ugt_competitor, ugt_primary_competitor_brands
from ugt_model import haversine_miles
...
from ugt_site_template import build_excel_report
try:
...
ugt_known_brands = list(set(
    ugt_primary_competitor_brands + [direct_competitor_brand] + anchor_brands + market_signal_brands + new_brands
))
...
def find_nearest_competitors(lat, lon, comp, n=n_competitors):
    comp = comp[comp["Brand"].isin(ugt_primary_competitor_brands)]
    if len(comp) == 0:
        return []
...
def find_co_tenants(lat, lon, comp, n=n_co_tenants):
    ...
    print(f"  Checking {len(stores)} stores in '{getattr(plaza, 'label', '?')}' against {len(ugt_known_brands)} tracked brands:")
    co_tenants = []
    for s in stores:
        matched_brand = _match_tenant_to_brand(s.name, ugt_known_brands)
...
def generate_site_report(address, used_names=None, manual_lat=None, manual_lon=None):
    ...
    print("Finding nearest existing UGT locations...")
    proximity = nearest_ugt_locations(lat, lon, n=n_nearest_stores)
    for p in proximity:
        print(f"  {p['name']}: {p['distance_mi']} mi")
 
    print("Loading competitor data...")
    ugt_comp = load_ugt_competitor()
    competitors = find_nearest_competitors(lat, lon, ugt_comp)
    ...
    combined_comp_for_cotenants = pd.concat([ugt_comp, tgg_comp], ignore_index=True)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print('usage: python ugt_site_report.py "<address>" [manual_lat manual_lon]')
        sys.exit(1)
    address = sys.argv[1]
