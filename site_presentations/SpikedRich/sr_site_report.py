...
from sr_proximity import nearest_sr_locations
from sr_model import load_sr_competitor, sr_primary_competitor_brands
from sr_model import haversine_miles
...
from sr_site_template import build_excel_report
try:
...
sr_known_brands = list(set(
    sr_primary_competitor_brands + [direct_competitor_brand] + anchor_brands + market_signal_brands + new_brands
))
...
def find_nearest_competitors(lat, lon, comp, n=n_competitors):
    comp = comp[comp["Brand"].isin(sr_primary_competitor_brands)]
    if len(comp) == 0:
        return []
...
def find_co_tenants(lat, lon, comp, n=n_co_tenants):
    ...
    print(f"  Checking {len(stores)} stores in '{getattr(plaza, 'label', '?')}' against {len(sr_known_brands)} tracked brands:")
    co_tenants = []
    for s in stores:
        matched_brand = _match_tenant_to_brand(s.name, sr_known_brands)
...
def generate_site_report(address, used_names=None):
    ...
    print("Finding nearest existing SR Protein locations...")
    proximity = nearest_sr_locations(lat, lon, n=n_nearest_stores)
    for p in proximity:
        print(f"  {p['name']}: {p['distance_mi']} mi")
 
    print("Loading competitor data...")
    sr_comp = load_sr_competitor()
    competitors = find_nearest_competitors(lat, lon, sr_comp)
    ...
    combined_comp_for_cotenants = pd.concat([sr_comp, tgg_comp], ignore_index=True)
