...
dark_blue = "FF123456" #back of main headers
 
night_blue = "FF151B54" # writing for address
 
light_blue_top = "FFD1E5F4" #where it says site presentation at top
 
light_blue_rest = "FFBDD5E7" #sub-header color
 
white = "FFFFFFFF"
 
very_light_blue = "FFE5F3FD" #for line spacing
 
title_cream = "FFFCE5CD"
...
fbanner_title = Font(name="Verdana", size=20, bold=True, color=white)
fbanner_sub = Font(name="Verdana", size=12, bold=True, color=night_blue)
faddress = Font(name="Georgia", size=14, bold=True, color=night_blue)
fsubtit = Font(name="Arial", size=10, bold=False, color=dark_blue)
fsection_head = Font(name="Verdana", size=11, bold=True, color=white)
ftable_head = Font(name="Arial", size=10, bold=True, color=night_blue)
fbody = Font(name="Roboto", size=10, bold=False)
 
fill_darkblue = PatternFill("solid", start_color=dark_blue)
fill_titlesub = PatternFill("solid", start_color=light_blue_top)
fill_lightblue = PatternFill("solid", start_color = light_blue_rest)
fill_verylightblue = PatternFill("solid", start_color = very_light_blue)
...
def _label_row(ws, row, label, *, stripe, value=None, value_align=leftc, number_format = None):
    fill = fill_verylightblue if stripe else None
    ws[f"B{row}"]=label
...
def _section_header(ws,row,text,span=("B","F")):
    ...
    for col_letter in [get_column_letter(c) for c in range(ord(span[0]) - 64, ord(span[1]) - 64 +1)]:
        ...
        ws[f"{col_letter}{row}"].fill = fill_darkblue
...
def _build_top_banner(ws, profile:dict, plaza_name: str|None=None):
    ...
    for col in ("B","C","D","E","F"):
        for row in (2,3):
            ...
            ws[f"{col}{row}"].fill = fill_darkblue
...
def _build_section_proximity(ws, proximity = None):
    ...
    for i, loc in enumerate(rows_data[:7]):
        ...
        _style_row(ws,row,fill = fill_verylightblue if stripe else None, cols = ("B","C","D","E","F"))
...
def _build_section_market(ws, competitors = None, co_tenants=None):
    ...
    ws["E32"].fill = fill_lightblue
    ws["E32"].font = ftable_head
    ws["E32"].alignment = center
    ws["E32"].border = box
    ws["F32"].fill = fill_lightblue
    ws["F32"].border = box
    ...
    ws["E38"].fill = fill_lightblue
    ws["E38"].font = ftable_head
    ws["E38"].alignment = center
    ws["E38"].border = box
    ws["F38"].fill = fill_lightblue
    ws["F38"].border = box
...
def _build_section_demographics(ws,profile):
    ...
    ws["E51"].fill = fill_lightblue
    ws["E51"].font = ftable_head
    ws["E51"].alignment = center
    ws["E51"].border = box
    ws["F51"].fill = fill_lightblue
    ws["F51"].border = box
    rows_spec = [
    ...
    rings = [
        ...
        ("D", profile.get("ring_3mi") or {}),
        ("E", profile.get("ring_5mi") or {}),
    ]
...
def build_excel_report(profile:dict, scores:dict, address:str, traffic:dict|None = None,proximity:list|None= None, competitors:list|None = None,
    ...
    base = f"Great Greek - {city_state} Site Summary"
