...
from pb_proximity import nearest_pinkberry_locations
from pb_model import load_pinkberry_competitor, pinkberry_primary_competitor_brands
from pb_model import haversine_miles
...
from pb_site_template import build_excel_report
try:
...
pinkberry_known_brands = list(set(
    pinkberry_primary_competitor_brands + [direct_competitor_brand] + anchor_brands + market_signal_brands + new_brands
))
...
def find_nearest_competitors(lat, lon, comp, n=n_competitors):
    comp = comp[comp["Brand"].isin(pinkberry_primary_competitor_brands)]
    if len(comp) == 0:
        return []
...
def find_co_tenants(lat, lon, comp, n=n_co_tenants):
    ...
    print(f"  Checking {len(stores)} stores in '{getattr(plaza, 'label', '?')}' against {len(pinkberry_known_brands)} tracked brands:")
    co_tenants = []
    for s in stores:
        matched_brand = _match_tenant_to_brand(s.name, pinkberry_known_brands)
...
def generate_site_report(address, used_names=None, manual_lat=None, manual_lon=None):
    """manual_lat/manual_lon are an OPTIONAL fallback, not a replacement --
    geocoding the typed address is always tried first. They only get used
    if that geocode fails..."""
    ...
    print("Finding nearest existing Pinkberry locations...")
    proximity = nearest_pinkberry_locations(lat, lon, n=n_nearest_stores)
    for p in proximity:
        print(f"  {p['name']}: {p['distance_mi']} mi")
 
    print("Loading competitor data...")
    pinkberry_comp = load_pinkberry_competitor()
    competitors = find_nearest_competitors(lat, lon, pinkberry_comp)
    ...
    combined_comp_for_cotenants = pd.concat([pinkberry_comp, tgg_comp], ignore_index=True)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print('usage: python st_site_report.py "<address>"')
        sys.exit(1)
    address = sys.argv[1]
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print('usage: python pb_site_report.py "<address>" [manual_lat manual_lon]')
        sys.exit(1)
    address = sys.argv[1]
