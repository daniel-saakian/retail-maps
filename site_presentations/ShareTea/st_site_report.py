...
from st_proximity import nearest_st_locations
from model_st import load_st_competitor, st_primary_competitor_brands
from model_st import haversine_miles
...
from st_site_template import build_excel_report
try:
...
st_known_brands = list(set(
    st_primary_competitor_brands + [direct_competitor_brand] + anchor_brands + market_signal_brands + new_brands
))
...
def find_nearest_competitors(lat, lon, comp, n=n_competitors):
    comp = comp[comp["Brand"].isin(st_primary_competitor_brands)]
    if len(comp) == 0:
        return []
...
def find_co_tenants(lat, lon, comp, n=n_co_tenants):
    ...
    print(f"  Checking {len(stores)} stores in '{getattr(plaza, 'label', '?')}' against {len(st_known_brands)} tracked brands:")
    co_tenants = []
    for s in stores:
        matched_brand = _match_tenant_to_brand(s.name, st_known_brands)
...
def generate_site_report(address, used_names=None, manual_lat=None, manual_lon=None):
    ...
    print("Finding nearest existing ShareTea locations...")
    proximity = nearest_st_locations(lat, lon, n=n_nearest_stores)
    for p in proximity:
        print(f"  {p['name']}: {p['distance_mi']} mi")
 
    print("Loading competitor data...")
    st_comp = load_st_competitor()
    competitors = find_nearest_competitors(lat, lon, st_comp)
    ...
    combined_comp_for_cotenants = pd.concat([st_comp, tgg_comp], ignore_index=True)
