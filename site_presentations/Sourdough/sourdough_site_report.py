...
from prox import nearest_sourdough_locations
from sourdough_model import load_sourdough_competitors, sourdough_primary_competitor_brands
from sourdough_model import haversine_miles
from find_single_plaza import find_single_plaza
from sourdough_site_template import build_excel_report
try:
...
# same 4-brand group treated as direct competitors for Sourdough - all
# tracked in the shared competitor DB, also used for co-tenant matching
sourdough_known_brands = list(sourdough_primary_competitor_brands)
...
def find_nearest_competitors(lat, lon, comp, n=n_competitors):
    comp = comp[comp["Brand"].isin(sourdough_primary_competitor_brands)]
    if len(comp) == 0:
        return []
...
def find_co_tenants(lat, lon, comp, n=n_co_tenants):
    ...
    print(f"  Checking {len(stores)} stores in '{getattr(plaza, 'label', '?')}' against {len(sourdough_known_brands)} tracked brands:")
    co_tenants = []
    for s in stores:
        matched_brand = _match_tenant_to_brand(s.name, sourdough_known_brands)
...
def generate_site_report(address, used_names=None, manual_lat=None, manual_lon=None):
    ...
    print("Finding nearest existing Sourdough & Co. locations...")
    proximity = nearest_sourdough_locations(lat, lon, n=n_nearest_stores)
    for p in proximity:
        print(f"  {p['name']}: {p['distance_mi']} mi")
 
    print("Loading competitor data...")
    comp = load_sourdough_competitors()
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print('usage: python st_site_report.py "<address>" [manual_lat manual_lon]')
        sys.exit(1)
    address = sys.argv[1]
