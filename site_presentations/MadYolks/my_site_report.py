...
from my_site_template import build_excel_report
try:
...
madyolks_known_brands = list(set(
    madyolks_primary_competitor_brands + [direct_competitor_brand] + anchor_brands + market_signal_brands + new_brands
))
...
def find_nearest_competitors(lat, lon, comp, n=n_competitors):
    comp = comp[comp["Brand"].isin(madyolks_primary_competitor_brands)]
    if len(comp) == 0:
        return []
...
def find_co_tenants(lat, lon, comp, n=n_co_tenants):
    ...
    print(f"  Checking {len(stores)} stores in '{getattr(plaza, 'label', '?')}' against {len(madyolks_known_brands)} tracked brands:")
    co_tenants = []
    for s in stores:
        matched_brand = _match_tenant_to_brand(s.name, madyolks_known_brands)
...
def generate_site_report(address, used_names=None):
    ...
    proximity = nearest_madyolks_locations(lat, lon, n=n_nearest_stores)
    for p in proximity:
        print(f"  {p['name']}: {p['distance_mi']} mi")
 
    print("Loading competitor data...")
    madyolks_comp = load_madyolks_competitor()
    competitors = find_nearest_competitors(lat, lon, madyolks_comp)
    ...
    combined_comp_for_cotenants = pd.concat([madyolks_comp, tgg_comp], ignore_index=True)
